...
private struct SupabaseHomeworkPublishWriteResult: Hashable {
    var title: String
    var status: String
    var statusColorName: String
    var method: String
    ...
    static func planned(config: SupabaseBackendConfig) -> SupabaseHomeworkPublishWriteResult {
        if config.hasClientApiKey == false {
            return missingKey(config: config)
        }
        ...
        return SupabaseHomeworkPublishWriteResult(
            title: "Homework publish write",
            status: "ready",
            statusColorName: "blue",
            method: "POST",
            path: "/homework_items",
            url: "\(config.restBaseURL)/homework_items",
            headerState: "apikey \(config.clientKeyKind) ready, manager bearer \(config.accessTokenPreview ?? "set")",
            detail: "Signed request will create homework only if RLS confirms teacher or parent-committee rights.",
            nextStep: "Refresh signed homework before replacing the local homework add form",
            targetPreview: homeworkTargetPreview
        )
    }

    static func missingKey(config: SupabaseBackendConfig) -> SupabaseHomeworkPublishWriteResult {
        SupabaseHomeworkPublishWriteResult(
            title: "Homework publish write",
            status: "blocked",
            statusColorName: "orange",
            method: "POST",
            ...
            url: "\(config.restBaseURL)/homework_items",
            headerState: "missing SUPABASE_PUBLISHABLE_KEY or SUPABASE_ANON_KEY",
            detail: "Signed homework publish write is blocked before network access.",
            nextStep: "Add client apikey, signed manager session and class bridge",
            targetPreview: homeworkTargetPreview
        )
    }

    static func missingAccessToken(config: SupabaseBackendConfig) -> SupabaseHomeworkPublishWriteResult {
        SupabaseHomeworkPublishWriteResult(
            title: "Homework publish write",
            status: "token missing",
            statusColorName: "orange",
            ...
            url: "\(config.restBaseURL)/homework_items",
            headerState: "apikey \(config.clientKeyKind) ready, missing SUPABASE_ACCESS_TOKEN",
            detail: "Client key alone cannot create class homework.",
            nextStep: "Inject a teacher or parent-committee Supabase access token before homework proof",
            targetPreview: homeworkTargetPreview
        )
    }

    static func missingUserID(config: SupabaseBackendConfig) -> SupabaseHomeworkPublishWriteResult {
        SupabaseHomeworkPublishWriteResult(
            title: "Homework publish write",
            status: "user id missing",
            statusColorName: "orange",
            ...
            url: "\(config.restBaseURL)/homework_items",
            headerState: "apikey \(config.clientKeyKind) ready, manager bearer \(config.accessTokenPreview ?? "set")",
            detail: "Access token exists, but the homework body needs the signed author id.",
            nextStep: "Provide SUPABASE_USER_ID for the manager seed user and retry homework write",
            targetPreview: homeworkTargetPreview
        )
    }

    static func missingClassContext(config: SupabaseBackendConfig) -> SupabaseHomeworkPublishWriteResult {
        SupabaseHomeworkPublishWriteResult(
            title: "Homework publish write",
            status: "class missing",
            statusColorName: "orange",
            ...
            url: "\(config.restBaseURL)/homework_items",
            headerState: "apikey \(config.clientKeyKind) ready, manager bearer \(config.accessTokenPreview ?? "set")",
            detail: "No Supabase class bridge item is available for homework.",
            nextStep: "Run signed class scope probe first, then retry homework write",
            targetPreview: homeworkTargetPreview
        )
    }

    static func success(config: SupabaseBackendConfig, body: SupabaseHomeworkPublishBody, statusCode: Int) -> SupabaseHomeworkPublishWriteResult {
        SupabaseHomeworkPublishWriteResult(
            title: "Homework publish write",
            status: "published",
            statusColorName: "green",
            ...
            url: "\(config.restBaseURL)/homework_items",
            headerState: "HTTP \(statusCode), manager bearer accepted",
            detail: "RLS accepted the teacher/committee homework row.",
            nextStep: "Refresh signed homework before replacing the local add form",
            targetPreview: "\(body.class_id), \(body.subject)"
        )
    }

    static func serverError(config: SupabaseBackendConfig, body: SupabaseHomeworkPublishBody?, statusCode: Int, message: String) -> SupabaseHomeworkPublishWriteResult {
        SupabaseHomeworkPublishWriteResult(
            title: "Homework publish write",
            status: "HTTP \(statusCode)",
            statusColorName: statusCode == 401 || statusCode == 403 ? "orange" : "red",
            ...
            headerState: statusCode == 401 || statusCode == 403 ? "manager auth/session required" : "\(config.clientKeyKind) apikey and manager bearer sent",
            detail: message,
            nextStep: statusCode == 401 || statusCode == 403 ? "Use teacher/committee session or verify homework_insert_manager RLS" : "Check homework_items Data API exposure and RLS response",
            targetPreview: body.map { "\($0.class_id), homework blocked" } ?? homeworkTargetPreview
        )
    }

    static func networkError(config: SupabaseBackendConfig, body: SupabaseHomeworkPublishBody?, message: String) -> SupabaseHomeworkPublishWriteResult {
        SupabaseHomeworkPublishWriteResult(
            title: "Homework publish write",
            status: "network",
            statusColorName: "red",
            ...
            headerState: "\(config.clientKeyKind) apikey and manager bearer prepared",
            detail: message,
            nextStep: "Keep local homework queued and retry after network/backend healthcheck",
            targetPreview: body.map { "\($0.class_id), homework queued" } ?? homeworkTargetPreview
        )
    }

    private static var homeworkTargetPreview: String {
        AppSupabaseClassContextBridge.primaryContext?.summary ?? "no saved class"
    }
}
...
private struct SupabaseCollectionExpenseWriteResult: Hashable {
    var title: String
    var status: String
    var statusColorName: String
    var method: String
    ...
    static func planned(config: SupabaseBackendConfig) -> SupabaseCollectionExpenseWriteResult {
        if config.hasClientApiKey == false {
            return missingKey(config: config)
        }
        ...
        return SupabaseCollectionExpenseWriteResult(
            title: "Collection expense write",
            status: "ready",
            statusColorName: "blue",
            method: "POST",
            ...
            headerState: "apikey \(config.clientKeyKind) ready, user bearer \(config.accessTokenPreview ?? "set")",
            detail: "Signed request will add a collection expense only if RLS confirms class manager rights.",
            nextStep: "Send expense write after signed upload/file metadata is ready for receipts",
            targetPreview: expenseTargetPreview
        )
    }

    static func missingKey(config: SupabaseBackendConfig) -> SupabaseCollectionExpenseWriteResult {
        SupabaseCollectionExpenseWriteResult(
            title: "Collection expense write",
            status: "blocked",
            statusColorName: "orange",
            method: "POST",
            ...
            url: "\(config.restBaseURL)/collection_expenses",
            headerState: "missing SUPABASE_PUBLISHABLE_KEY or SUPABASE_ANON_KEY",
            detail: "Signed collection expense write is blocked before network access.",
            nextStep: "Add client apikey, signed manager session and collection bridge",
            targetPreview: expenseTargetPreview
        )
    }

    static func missingAccessToken(config: SupabaseBackendConfig) -> SupabaseCollectionExpenseWriteResult {
        SupabaseCollectionExpenseWriteResult(
            title: "Collection expense write",
            status: "token missing",
            statusColorName: "orange",
            ...
            url: "\(config.restBaseURL)/collection_expenses",
            headerState: "apikey \(config.clientKeyKind) ready, missing SUPABASE_ACCESS_TOKEN",
            detail: "Client key alone cannot add collection expenses.",
            nextStep: "Inject a teacher or parent-committee Supabase access token before expense proof",
            targetPreview: expenseTargetPreview
        )
    }

    static func missingUserID(config: SupabaseBackendConfig) -> SupabaseCollectionExpenseWriteResult {
        SupabaseCollectionExpenseWriteResult(
            title: "Collection expense write",
            status: "user id missing",
            statusColorName: "orange",
            ...
            nextStep: "Provide SUPABASE_USER_ID for the manager seed user and retry expense write",
            targetPreview: expenseTargetPreview
        )
    }

    static func missingCollection(config: SupabaseBackendConfig) -> SupabaseCollectionExpenseWriteResult {
        SupabaseCollectionExpenseWriteResult(
            title: "Collection expense write",
            status: "collection missing",
            statusColorName: "orange",
            ...
            nextStep: "Run signed collections probe first, then retry expense write",
            targetPreview: expenseTargetPreview
        )
    }

    static func success(config: SupabaseBackendConfig, body: SupabaseCollectionExpenseBody, statusCode: Int) -> SupabaseCollectionExpenseWriteResult {
        SupabaseCollectionExpenseWriteResult(
            title: "Collection expense write",
            status: "saved",
            statusColorName: "green",
            ...
            url: "\(config.restBaseURL)/collection_expenses",
            headerState: "HTTP \(statusCode), manager bearer accepted",
            detail: "RLS accepted the class manager expense row.",
            nextStep: "Refresh collections and expenses before replacing the local receipt form",
            targetPreview: "\(body.collection_id), \(body.title), amount \(body.amount)"
        )
    }

    static func serverError(config: SupabaseBackendConfig, body: SupabaseCollectionExpenseBody?, statusCode: Int, message: String) -> SupabaseCollectionExpenseWriteResult {
        SupabaseCollectionExpenseWriteResult(
            title: "Collection expense write",
            status: "HTTP \(statusCode)",
            statusColorName: statusCode == 401 || statusCode == 403 ? "orange" : "red",
            ...
            path: "/collection_expenses",
            url: "\(config.restBaseURL)/collection_expenses",
            headerState: statusCode == 401 || statusCode == 403 ? "manager auth/session required" : "\(config.clientKeyKind) apikey and user bearer sent",
            detail: message,
            nextStep: statusCode == 401 || statusCode == 403 ? "Use teacher/committee session or verify can_manage_class RLS" : "Check collection_expenses Data API exposure and RLS response",
            targetPreview: body.map { "\($0.collection_id), \($0.title), blocked" } ?? expenseTargetPreview
        )
    }

    static func networkError(config: SupabaseBackendConfig, body: SupabaseCollectionExpenseBody?, message: String) -> SupabaseCollectionExpenseWriteResult {
        SupabaseCollectionExpenseWriteResult(
            title: "Collection expense write",
            status: "network",
            statusColorName: "red",
            ...
            headerState: "\(config.clientKeyKind) apikey and user bearer prepared",
            detail: message,
            nextStep: "Keep local expense queued and retry after network/backend healthcheck",
            targetPreview: body.map { "\($0.collection_id), \($0.title)" } ?? expenseTargetPreview
        )
    }

    private static var expenseTargetPreview: String {
        let collection = AppSupabaseCollectionBridge.primaryCollection?.summary ?? "no saved collection"
        return "\(collection) -> Чек за автобус [2500]"
    }
}
...
private struct SupabaseClassFileMetadataWriteResult: Hashable {
    ...
    static func planned(config: SupabaseBackendConfig) -> SupabaseClassFileMetadataWriteResult {
        if config.hasClientApiKey == false {
            return missingKey(config: config)
        }
        ...
        return SupabaseClassFileMetadataWriteResult(
            title: "Class file metadata write",
            status: "ready",
            statusColorName: "blue",
            method: "POST",
            path: "/class_files",
            url: "\(config.restBaseURL)/class_files",
            headerState: "apikey \(config.clientKeyKind) ready, user bearer \(config.accessTokenPreview ?? "set")",
            detail: "Signed request will save receipt/photo metadata after a private storage upload.",
            nextStep: "Use returned file id for receipt_file_id or class_photos.file_id in the final upload flow",
            targetPreview: fileTargetPreview
        )
    }

    static func missingKey(config: SupabaseBackendConfig) -> SupabaseClassFileMetadataWriteResult {
        SupabaseClassFileMetadataWriteResult(
            title: "Class file metadata write",
            status: "blocked",
            statusColorName: "orange",
            method: "POST",
            ...
            url: "\(config.restBaseURL)/class_files",
            headerState: "missing SUPABASE_PUBLISHABLE_KEY or SUPABASE_ANON_KEY",
            detail: "Signed class file metadata write is blocked before network access.",
            nextStep: "Add client apikey, signed session, user id and class bridge",
            targetPreview: fileTargetPreview
        )
    }

    static func missingAccessToken(config: SupabaseBackendConfig) -> SupabaseClassFileMetadataWriteResult {
        SupabaseClassFileMetadataWriteResult(
            title: "Class file metadata write",
            status: "token missing",
            statusColorName: "orange",
            ...
            url: "\(config.restBaseURL)/class_files",
            headerState: "apikey \(config.clientKeyKind) ready, missing SUPABASE_ACCESS_TOKEN",
            detail: "Client key alone cannot save private file metadata.",
            nextStep: "Inject a signed Supabase access token before file metadata proof",
            targetPreview: fileTargetPreview
        )
    }

    static func missingUserID(config: SupabaseBackendConfig) -> SupabaseClassFileMetadataWriteResult {
        SupabaseClassFileMetadataWriteResult(
            title: "Class file metadata write",
            status: "user id missing",
            statusColorName: "orange",
            ...
            url: "\(config.restBaseURL)/class_files",
            headerState: "apikey \(config.clientKeyKind) ready, user bearer \(config.accessTokenPreview ?? "set")",
            detail: "Access token exists, but the file owner id is required.",
            nextStep: "Provide SUPABASE_USER_ID for the signed user and retry metadata write",
            targetPreview: fileTargetPreview
        )
    }

    static func missingClassContext(config: SupabaseBackendConfig) -> SupabaseClassFileMetadataWriteResult {
        SupabaseClassFileMetadataWriteResult(
            title: "Class file metadata write",
            status: "class missing",
            statusColorName: "orange",
            ...
            url: "\(config.restBaseURL)/class_files",
            headerState: "apikey \(config.clientKeyKind) ready, user bearer \(config.accessTokenPreview ?? "set")",
            detail: "No Supabase class bridge item is available for the file row.",
            nextStep: "Run signed class scope probe first, then retry file metadata write",
            targetPreview: fileTargetPreview
        )
    }

    static func success(config: SupabaseBackendConfig, body: SupabaseClassFileMetadataBody, statusCode: Int) -> SupabaseClassFileMetadataWriteResult {
        SupabaseClassFileMetadataWriteResult(
            title: "Class file metadata write",
            status: "saved",
            statusColorName: "green",
            ...
            url: "\(config.restBaseURL)/class_files",
            headerState: "HTTP \(statusCode), user bearer accepted",
            detail: "RLS accepted the class member file metadata row.",
            nextStep: "Attach this file id to a receipt expense or class photo row after storage upload",
            targetPreview: "\(body.kind), \(body.file_name), \(body.object_path)"
        )
    }

    static func duplicate(config: SupabaseBackendConfig, body: SupabaseClassFileMetadataBody, statusCode: Int) -> SupabaseClassFileMetadataWriteResult {
        SupabaseClassFileMetadataWriteResult(
            title: "Class file metadata write",
            status: "already saved",
            statusColorName: "green",
            ...
            path: "/class_files",
            url: "\(config.restBaseURL)/class_files",
            headerState: "HTTP \(statusCode), unique bucket/object_path held",
            detail: "The same private storage object metadata already exists and is treated as idempotent success.",
            nextStep: "Fetch the existing file row before linking it to receipt or photo metadata",
            targetPreview: "\(body.kind), duplicate-safe"
        )
    }

    static func serverError(config: SupabaseBackendConfig, body: SupabaseClassFileMetadataBody?, statusCode: Int, message: String) -> SupabaseClassFileMetadataWriteResult {
        SupabaseClassFileMetadataWriteResult(
            title: "Class file metadata write",
            status: "HTTP \(statusCode)",
            statusColorName: statusCode == 401 || statusCode == 403 ? "orange" : "red",
            ...
            headerState: statusCode == 401 || statusCode == 403 ? "auth/session required" : "\(config.clientKeyKind) apikey and user bearer sent",
            detail: message,
            nextStep: statusCode == 401 || statusCode == 403 ? "Refresh seed session or verify class membership RLS" : "Check class_files Data API exposure, object_path uniqueness and RLS response",
            targetPreview: body.map { "\($0.kind), \($0.file_name), blocked" } ?? fileTargetPreview
        )
    }

    static func networkError(config: SupabaseBackendConfig, body: SupabaseClassFileMetadataBody?, message: String) -> SupabaseClassFileMetadataWriteResult {
        SupabaseClassFileMetadataWriteResult(
            title: "Class file metadata write",
            status: "network",
            statusColorName: "red",
            ...
            headerState: "\(config.clientKeyKind) apikey and user bearer prepared",
            detail: message,
            nextStep: "Keep local file queued and retry after network/backend healthcheck",
            targetPreview: body.map { "\($0.kind), \($0.file_name)" } ?? fileTargetPreview
        )
    }

    private static var fileTargetPreview: String {
        let classContext = AppSupabaseClassContextBridge.primaryContext?.summary ?? "no saved class"
        return "\(classContext) -> receipt QA file metadata"
    }
}
...
private struct SupabaseHomeworkPublishBody: Encodable, Hashable {
    var class_id: String
    var author_user_id: String
    var subject: String
    var title: String
...
private struct SupabaseHomeworkPublishWriteClient {
    static func publishProbeHomework(config: SupabaseBackendConfig) async -> SupabaseHomeworkPublishWriteResult {
        guard let apiKey = config.clientApiKey, apiKey.isEmpty == false else {
            return .missingKey(config: config)
        }
        ...
        let body = SupabaseHomeworkPublishBody(
            class_id: classContext.classID,
            author_user_id: userID,
            subject: "Математика",
            title: "QA задание из Sync Center",
            details: "Проверка signed homework publish gate из iOS.",
            due_at: "2026-07-20T15:00:00Z",
            assignee_child_id: nil
        )

        guard let url = URL(string: "\(config.restBaseURL)/homework_items") else {
            return .networkError(config: config, body: body, message: "Invalid Supabase homework_items URL")
        }
...
private struct SupabaseCollectionExpenseBody: Encodable, Hashable {
    var collection_id: String
    var author_user_id: String
    var title: String
    var amount: String
...
private struct SupabaseCollectionExpenseWriteClient {
    static func saveProbeExpense(config: SupabaseBackendConfig) async -> SupabaseCollectionExpenseWriteResult {
        guard let apiKey = config.clientApiKey, apiKey.isEmpty == false else {
            return .missingKey(config: config)
        }
        ...
        let body = SupabaseCollectionExpenseBody(
            collection_id: collection.id,
            author_user_id: userID,
            title: "QA чек за автобус",
            amount: "2500"
        )
...
private struct SupabaseClassFileMetadataWriteClient {
    static func saveProbeMetadata(config: SupabaseBackendConfig) async -> SupabaseClassFileMetadataWriteResult {
        guard let apiKey = config.clientApiKey, apiKey.isEmpty == false else {
            return .missingKey(config: config)
        }
        ...
        let body = SupabaseClassFileMetadataBody(
            class_id: classContext.classID,
            owner_user_id: userID,
            kind: "receipt",
            bucket: config.storageBucket,
            object_path: "qa/\(classContext.classID)/receipts/ios-receipt-metadata-probe.pdf",
            file_name: "ios-receipt-metadata-probe.pdf",
            mime_type: "application/pdf",
            size_bytes: 120_000
        )

        guard let url = URL(string: "\(config.restBaseURL)/class_files") else {
            return .networkError(config: config, body: body, message: "Invalid Supabase class_files URL")
        }
...
private struct SyncCenterSheet: View {
    ...
    @State private var supabaseHomeworkPublishWrite = SupabaseHomeworkPublishWriteResult.planned(config: SupabaseBackendConfig.make())
    ...
    init(operations: [SyncOperationSummary], onSave: @escaping ([SyncOperationSummary]) -> Void) {
        ...
        _supabaseHomeworkPublishWrite = State(initialValue: SupabaseHomeworkPublishWriteResult.planned(config: launchSupabaseConfig))
    ...
    private var supabaseReadinessCard: some View {
        ...
        return VStack(alignment: .leading, spacing: 12) {
            ...
            supabaseHomeworkPublishWriteCard(supabaseHomeworkPublishWrite)
    ...
    private func supabaseHomeworkPublishWriteCard(_ result: SupabaseHomeworkPublishWriteResult) -> some View {
        VStack(alignment: .leading, spacing: 7) {
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                Label(result.title, systemImage: "book.closed.fill")
                    .font(.caption.weight(.bold))
            ...
            Text("Homework: \(result.targetPreview)")
                .font(.caption2.monospaced())
                .foregroundStyle(SchoolTheme.graphite.opacity(0.72))
                .lineLimit(3)
                .minimumScaleFactor(0.52)
            ...
            Button {
                Task {
                    await runSupabaseHomeworkPublishWrite()
                }
            } label: {
                Label("Опубликовать ДЗ", systemImage: "book.closed.fill")
                    .font(.caption.weight(.semibold))
                    .foregroundStyle(SchoolTheme.success)
                    .frame(maxWidth: .infinity, minHeight: 38)
                    .background(SchoolTheme.success.opacity(0.11), in: Capsule())
            ...
            .buttonStyle(.plain)
            .accessibilityIdentifier("sync.supabase-homework-publish-write")
        }
        .padding(10)
        .background(moreColor(for: result.statusColorName).opacity(0.08), in: RoundedRectangle(cornerRadius: 8, style: .continuous))
    }
    ...
    private func supabaseCollectionExpenseWriteCard(_ result: SupabaseCollectionExpenseWriteResult) -> some View {
        VStack(alignment: .leading, spacing: 7) {
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                Label(result.title, systemImage: "receipt.fill")
                    .font(.caption.weight(.bold))
                    .foregroundStyle(SchoolTheme.graphite)
                Spacer()
                StatusBadge(text: result.status, color: moreColor(for: result.statusColorName))
            ...
            Text("Expense: \(result.targetPreview)")
                .font(.caption2.monospaced())
                .foregroundStyle(SchoolTheme.graphite.opacity(0.72))
                .lineLimit(3)
                .minimumScaleFactor(0.58)
            ...
            Button {
                Task {
                    await runSupabaseCollectionExpenseWrite()
                }
            } label: {
                Label("Записать расход сбора", systemImage: "receipt.fill")
                    .font(.caption.weight(.semibold))
                    .foregroundStyle(SchoolTheme.success)
                    .frame(maxWidth: .infinity, minHeight: 38)
                    .background(SchoolTheme.success.opacity(0.11), in: Capsule())
            ...
            .buttonStyle(.plain)
            .accessibilityIdentifier("sync.supabase-collection-expense-write")
        }
        .padding(10)
        .background(moreColor(for: result.statusColorName).opacity(0.08), in: RoundedRectangle(cornerRadius: 8, style: .continuous))
    }
    ...
    private func supabaseClassFileMetadataWriteCard(_ result: SupabaseClassFileMetadataWriteResult) -> some View {
        VStack(alignment: .leading, spacing: 7) {
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                Label(result.title, systemImage: "doc.badge.plus")
                    .font(.caption.weight(.bold))
                    .foregroundStyle(SchoolTheme.graphite)
                Spacer()
                StatusBadge(text: result.status, color: moreColor(for: result.statusColorName))
            ...
            Text("File: \(result.targetPreview)")
                .font(.caption2.monospaced())
                .foregroundStyle(SchoolTheme.graphite.opacity(0.72))
                .lineLimit(3)
                .minimumScaleFactor(0.54)
            ...
            Button {
                Task {
                    await runSupabaseClassFileMetadataWrite()
                }
            } label: {
                Label("Записать файл", systemImage: "doc.badge.plus")
                    .font(.caption.weight(.semibold))
                    .foregroundStyle(SchoolTheme.success)
                    .frame(maxWidth: .infinity, minHeight: 38)
                    .background(SchoolTheme.success.opacity(0.11), in: Capsule())
            ...
            .buttonStyle(.plain)
            .accessibilityIdentifier("sync.supabase-class-file-metadata-write")
        }
        .padding(10)
        .background(moreColor(for: result.statusColorName).opacity(0.08), in: RoundedRectangle(cornerRadius: 8, style: .continuous))
    }
    ...
    private func reloadSupabaseProbeState() {
        ...
        supabaseHomeworkPublishWrite = SupabaseHomeworkPublishWriteResult.planned(config: supabaseConfig)
    ...
    private func runSupabasePasswordSignInProbe() async {
        ...
        supabaseHomeworkPublishWrite = SupabaseHomeworkPublishWriteResult.planned(config: signedConfig)
    ...
    @MainActor
    private func runSupabaseHomeworkPublishWrite() async {
        reloadSupabaseProbeState()
        syncStatus = "Supabase homework publish: готовлю signed POST /homework_items для учителя/родкомитета."
        let result = await SupabaseHomeworkPublishWriteClient.publishProbeHomework(config: supabaseConfig)
        supabaseHomeworkPublishWrite = result
        syncStatus = result.status == "blocked" || result.status == "token missing" || result.status == "user id missing" || result.status == "class missing"
            ? "Supabase homework publish заблокирован: нужен client key, SUPABASE_ACCESS_TOKEN, SUPABASE_USER_ID и class bridge."
            : "Supabase homework publish завершен: \(result.headerState). \(result.nextStep)"
    }
    ...
    @MainActor
    private func runSupabaseCollectionExpenseWrite() async {
        reloadSupabaseProbeState()
        syncStatus = "Supabase collection expense: готовлю signed POST /collection_expenses для расхода родкомитета/учителя."
        let result = await SupabaseCollectionExpenseWriteClient.saveProbeExpense(config: supabaseConfig)
        supabaseCollectionExpenseWrite = result
        syncStatus = result.status == "blocked" || result.status == "token missing" || result.status == "user id missing" || result.status == "collection missing"
            ? "Supabase collection expense заблокирован: нужен client key, SUPABASE_ACCESS_TOKEN, SUPABASE_USER_ID и collection bridge."
            : "Supabase collection expense завершен: \(result.headerState). \(result.nextStep)"
    }
