...
| Backend / синхронизация | `[~]` | 2 | Для MVP добавлены локальное хранение, backend-контракты, OpenAPI draft, экран очереди синхронизации, typed dry-run API-клиента, auth context и readiness-контроль API; настоящий backend, storage и серверные права еще не подключены. |
