...
| Backend / синхронизация | `[~]` | 2 | Для MVP добавлены локальное хранение, backend-контракты, OpenAPI draft, экран очереди синхронизации, typed dry-run API-клиента и readiness-контроль API; настоящий backend, auth, storage и серверные права еще не подключены. |
