...
| Backend / синхронизация | `[~]` | 2 | Для MVP добавлены локальное хранение, backend-контракты, OpenAPI draft, signed upload contract, экран очереди синхронизации, typed dry-run API-клиента, auth context, storage preflight и readiness-контроль API; настоящий backend, storage и серверные права еще не подключены. |
