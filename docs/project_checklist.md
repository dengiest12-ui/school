...
| Backend / синхронизация | `[~]` | 2 | Для MVP добавлены локальное хранение, backend-контракты, OpenAPI draft, экран очереди синхронизации и readiness-контроль API; настоящий backend, API-клиент, storage и серверные права еще не подключены. |
