...
- [~] Автоматические тесты критичных сценариев
  ...
  - Комментарий: добавлен запускаемый smoke-скрипт: сборка, установка в Simulator, перезапуск приложения между сценариями и снимки ключевых экранов; каждый снимок проверяется на наличие, PNG-размеры и минимальный вес файла, а прогон пишет summary. Добавлен XCTest/UI target: тесты кликают/запускают приложение с QA-аргументами и проверяют детский режим, запрет родителю создавать сбор, Behavior QA gate, сохранение подтверждения "Я прочитал" и сохранение расхода сбора после перезапуска приложения. Перед релизом нужно расширить XCTest на создание ДЗ/событий и серверные права.
