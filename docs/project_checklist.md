...
- [~] Спроектировать API / контракты синхронизации
  ...
  - Уровень: 1
  - Артефакт: `docs/backend_contracts.md`
  - Комментарий: зафиксированы формат мутаций, подтверждения сервера, optimistic state, offline-очередь и правила конфликтов; OpenAPI/реальный клиент еще не реализованы
...
| 2026-07-03 | Smoke-проверки критичных сценариев | Пройдено | 2 | `scripts/qa_smoke.sh`, `.build/screenshots/qa-smoke/` | Добавлен и прогнан скрипт сборки/установки/запуска 7 QA-сценариев в Simulator; исправлены перезапуск приложения между кейсами и аргумент вкладки `classRoom`; сборка `xcodebuild` проходит |
