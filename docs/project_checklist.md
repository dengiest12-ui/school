...
| 2026-07-10 | Supabase announcement publish write gate | Пройдено | 4 | `SchoolApp/Features/More/MoreView.swift`, `SchoolAppUITests/SchoolAppUITests.swift`, `docs/project_checklist.md`, `docs/supabase/test_backend_plan.md`, `.build/SupabaseAnnouncementPublishWriteUITest.xcresult` | В Sync Center добавлен signed `POST /announcements` gate для публикации объявлений учителем или родкомитетом: тело содержит `class_id`, `author_user_id`, title/body и `is_urgent`, а право записи остается за RLS `announcements_insert_manager`; родительский сценарий не получает этот серверный доступ; без client key/session/user id/class bridge запрос блокируется до сети; `xcodebuild build` прошел, точечный UI-тест `testSupabaseAnnouncementPublishWriteBlocksBeforeClientKey` прошел |
| 2026-07-10 | Supabase announcement update write gate | Пройдено | 4 | `SchoolApp/Features/More/MoreView.swift`, `SchoolAppUITests/SchoolAppUITests.swift`, `docs/project_checklist.md`, `docs/supabase/test_backend_plan.md`, `.build/SupabaseAnnouncementUpdateWriteUITest.xcresult` | В Sync Center добавлен signed `PATCH /announcements?id=eq.<announcement>` gate для редактирования существующего объявления: тело меняет только title/body/`is_urgent`, не переносит объявление между классами и не меняет автора, а право записи остается за RLS `announcements_update_manager`; без client key/session/announcement bridge запрос блокируется до сети; `xcodebuild build` прошел, точечный UI-тест `testSupabaseAnnouncementUpdateWriteBlocksBeforeClientKey` прошел |
...
| 2026-07-10 | Supabase homework publish write gate | Пройдено | 4 | `SchoolApp/Features/More/MoreView.swift`, `SchoolAppUITests/SchoolAppUITests.swift`, `docs/project_checklist.md`, `docs/supabase/test_backend_plan.md`, `.build/SupabaseHomeworkPublishWriteUITest.xcresult` | В Sync Center добавлен signed `POST /homework_items` gate для создания ДЗ учителем или родкомитетом: тело содержит `class_id`, `author_user_id`, subject/title/details/due_at, а право записи остается за RLS `homework_insert_manager`; родительский сценарий не получает серверный доступ; без client key/session/user id/class bridge запрос блокируется до сети; `xcodebuild build` прошел, точечный UI-тест `testSupabaseHomeworkPublishWriteBlocksBeforeClientKey` прошел |
...
| 2026-07-10 | Supabase receipt-linked expense write gate | Пройдено | 4 | `SchoolApp/Models/SampleData.swift`, `SchoolApp/Features/More/MoreView.swift`, `SchoolAppUITests/SchoolAppUITests.swift`, `docs/project_checklist.md`, `docs/supabase/test_backend_plan.md`, `.build/SupabaseCollectionReceiptExpenseWriteUITest.xcresult` | В Sync Center добавлен signed `POST /collection_expenses` gate для расхода с привязанным `receipt_file_id`: приложение требует client key, manager bearer, user id, collection bridge и receipt `class_files` bridge, а право записи остается за RLS `can_manage_class`; QA-seed class_files разделяет receipt и class_photo, чтобы фото и чеки не путали file id; без client key/session запрос блокируется до сети; `xcodebuild build` прошел, точечный UI-тест `testSupabaseCollectionReceiptExpenseWriteBlocksBeforeClientKey` прошел |
