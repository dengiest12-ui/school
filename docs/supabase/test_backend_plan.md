...
- Keep the local announcement feed active until signed read, publish, edit and read-ack flows can switch through one repository; publish/edit/read-ack now have separate signed write gates.
...
- The sync-center now has a signed announcement update gate: without client key/session/announcement bridge it blocks before network, and with a signed teacher or parent-committee session it prepares `PATCH /announcements?id=eq.<announcement>` for title/body/urgent edits while RLS `announcements_update_manager` remains the authority.
