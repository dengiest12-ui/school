...
2. Запросить у backend signed upload URL для приватного bucket.
...
Текущий iOS dry-run уже показывает этот preflight в `Еще -> Синхронизация`, но настоящий backend все равно должен выдать signed URL, проверить права класса/семьи и сохранить audit event.
