...
Текущий iOS dry-run уже показывает этот preflight и upload-intent preview в `Еще -> Синхронизация`, а OpenAPI draft фиксирует контракт `POST /files/upload-url`. Настоящий backend все равно должен выдать signed URL, проверить права класса/семьи, сохранить audit event и выполнить file scan/moderation до публикации.
