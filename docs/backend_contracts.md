...
2. Запросить у backend signed upload URL через `POST /files/upload-url`.
3. Загрузить binary/file в private storage.
4. Получить `fileId`, checksum, размер и mime type.
5. Отправить metadata-мутацию в `POST /sync/mutations` уже со ссылкой на `fileId`.

Текущий iOS dry-run уже показывает этот preflight в `Еще -> Синхронизация`, а OpenAPI draft фиксирует контракт `POST /files/upload-url`. Настоящий backend все равно должен выдать signed URL, проверить права класса/семьи, сохранить audit event и выполнить file scan/moderation до публикации.
