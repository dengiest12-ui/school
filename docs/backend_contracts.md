...
- собирает preview batch-запроса `POST /sync/mutations` с base URL окружения, auth-заглушкой, idempotency key и компактным телом;
- фиксирует scaffold будущего Swift-клиента: `URLSession`, result mapping, retry-план, server version persistence и остановку автоматической отправки для storage/conflict/auth блокеров;
...
### iOS SyncClient scaffold

Следующий слой в приложении должен заменить dry-run на реальный transport. Текущий экран уже фиксирует, что клиенту нужно:

- отправлять batch через `URLSession` и декодировать `MutationBatchResponse`;
