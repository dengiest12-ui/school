...
Формальный черновик API зафиксирован отдельно в `docs/openapi_mvp.yaml`. Он покрывает первые endpoint-ы, которые уже отображаются в iOS-центре синхронизации: healthcheck окружения, entitlement подписки, экспорт/удаление данных, создание класса, публикацию ДЗ, подтверждение прочтения объявления, чек сбора, приглашение, фото альбома и batch-отправку offline-мутаций.
...
- показывает metadata release preview: после `clean` заменяет `pending-upload` на `fileId` и разблокирует metadata-мутацию;
- показывает network readiness: live-mode gate, `GET /health`, timeout/retry policy, auth refresh и запрет локального обхода `403`;
...
- проверять выбранное окружение через `GET /health` до live-синхронизации;
...
Пока backend не поднят, iOS остается в dry-run. Переключение на live-синхронизацию допускается только после прохождения gate:

1. `GET /health` возвращает `status = ok`, корректный `environment`, `liveSyncEnabled = true` и `storageEnabled = true`.
2. `POST /sync/mutations` отвечает typed `MutationBatchResponse` и сохраняет идемпотентность `mutationId`.
3. `401` делает один refresh token retry; повторный `401` выводит пользователя из сессии.
4. `403` всегда становится role blocker, UI не имеет права локально обойти запрет.
5. Network/5xx ошибки остаются в offline-очереди с exponential backoff.
6. Таймауты клиента: 8 секунд для обычного API-запроса, 30 секунд для upload-запроса.

Перед TestFlight staging должен пройти этот gate вместе с auth refresh, file scan и metadata release.
...
7. Сформировать metadata release: заменить `pending-upload` на `fileId` и приложить `scanStatus = clean`.
8. Отправить metadata-мутацию в `POST /sync/mutations` уже со ссылкой на `fileId`.

Текущий iOS dry-run уже показывает этот preflight, upload-intent preview, mock signed-response preview, scan gate и metadata release preview в `Еще -> Синхронизация`, а OpenAPI draft фиксирует контракт `POST /files/upload-url`. Настоящий backend все равно должен выдать signed URL, проверить права класса/семьи, сохранить audit event и выполнить file scan/moderation до публикации.
...
Перед удалением клиент должен показать экспорт:

1. `GET /me/export?scope=...` возвращает export manifest: секции, срок жизни и ссылку на скачивание, если файл готов.
2. Пользователь подтверждает удаление после просмотра scope.
3. `POST /me/deletion-requests` создает заявку, пишет `AuditLog` и возвращает `requestId`.
4. `GET /me/deletion-requests/{requestId}` показывает статус заявки, `canCancel` и срок grace period.
5. Данные ребенка, файлы и семейные связи скрываются сразу по scope, удаление аккаунта получает 7 дней grace period.
6. `POST /me/deletion-requests/{requestId}/cancel` доступен только до конца grace period и только после повторной авторизации.
7. Создание и отмена заявки всегда пишут `AuditLog`; при отмене сервер восстанавливает доступы только в пределах исходного scope.
