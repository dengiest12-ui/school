...
Формальный черновик API зафиксирован отдельно в `docs/openapi_mvp.yaml`. Он покрывает первые endpoint-ы, которые уже отображаются в iOS-центре синхронизации: healthcheck окружения, создание класса, публикацию ДЗ, подтверждение прочтения объявления, чек сбора, приглашение, фото альбома и batch-отправку offline-мутаций.
